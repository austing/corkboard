/**
 * TreeScrapCard Component
 *
 * Displays a scrap in a tree view with uniform sizing.
 * All scraps are the same size regardless of visibility status.
 * Invisible scraps use the same styling as in regular views.
 *
 * @example
 * ```tsx
 * <TreeScrapCard
...
export function TreeScrapCard({
  ...
  return (
    <div
      id={scrap.code}
      className={`shadow-lg rounded-lg p-4 w-80 h-80 flex flex-col transition-shadow ${isClickable ? 'cursor-pointer' : 'cursor-default'} ${getCardClassName()}`}
    ...
    >
      ...
      <div className="mb-3 flex-grow overflow-hidden">
        <div
          className={`text-sm line-clamp-[14] froala-content ${isInvisibleScrap ? config.theme.invisible.textColor : ''}`}
