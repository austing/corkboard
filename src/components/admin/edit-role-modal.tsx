...
export default function EditRoleModal({ isOpen, role, onClose, onRoleUpdated }: EditRoleModalProps) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          permissionIds: data.role.permissions.map((p: { id: string; name: string }) => p.id),
        });
      }
    } catch (err) {
